...
final class BatteryPolicyTests: XCTestCase {
  func testDesiredMode_ControlDisabled_ReturnsNormal() {
    ...
    let mode = policy.desiredMode(
      currentCharge: 90, settings: settings(enabled: false, limit: 80), lastAppliedMode: nil)
  ...
  func testDesiredMode_ControlEnabledBelowUpperThreshold_ReturnsChargeLimit() {
    ...
    let mode = policy.desiredMode(
      currentCharge: 80, settings: settings(enabled: true, limit: 80), lastAppliedMode: nil)
  ...
  func testDesiredMode_ControlEnabledAtUpperThreshold_ReturnsHold() {
    let policy = BatteryPolicy(hysteresisPercent: 1)
    let mode = policy.desiredMode(
      currentCharge: 81, settings: settings(enabled: true, limit: 80), lastAppliedMode: nil)
  ...
  func testDesiredMode_LastModeHoldAboveLowerThreshold_RemainsHold() {
    let policy = BatteryPolicy(hysteresisPercent: 1)
    let mode = policy.desiredMode(
      currentCharge: 80, settings: settings(enabled: true, limit: 80), lastAppliedMode: .hold(90))
  ...
  func testDesiredMode_LastModeHoldAtLowerThreshold_SwitchesToChargeLimit() {
    let policy = BatteryPolicy(hysteresisPercent: 1)
    let mode = policy.desiredMode(
      currentCharge: 79, settings: settings(enabled: true, limit: 80), lastAppliedMode: .hold(90))
    XCTAssertEqual(mode, .chargeLimit(80))
  }
  ...
  func testDesiredMode_ChargeLimitBelowMinimum_ClampsToMinLimit() {
    let policy = BatteryPolicy(hysteresisPercent: 1)
    let mode = policy.desiredMode(
      currentCharge: 40, settings: settings(enabled: true, limit: 20), lastAppliedMode: nil)
  ...
  func testDesiredMode_ChargeLimitAboveMaximum_ClampsToMaxLimit() {
    let policy = BatteryPolicy(hysteresisPercent: 1)
    let mode = policy.desiredMode(
      currentCharge: 99, settings: settings(enabled: true, limit: 120), lastAppliedMode: nil)
